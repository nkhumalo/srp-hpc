...
if [ -f job_list_h2_6_31g.txt ]
then
  rm job_list_h2_6_31g.txt
fi
if [ -f job_list_h2_sto_3g.txt ]
then
  rm job_list_h2_sto_3g.txt
fi
if [ -f results_h2_hf_6_31g.dat ]
then
  rm results_h2_hf_6_31g.dat
fi
if [ -f results_h2_hf_sto_3g.dat ]
then
  rm results_h2_hf_sto_3g.dat
fi
if [ -f results_h2_mc_6_31g.dat ]
then
  rm results_h2_mc_6_31g.dat
fi
if [ -f results_h2_mc_sto_3g.dat ]
then
  rm results_h2_mc_sto_3g.dat
fi
...
task mcscf
EOF
  #$NWCHEM_EXE input-h2-mc-6-31g.nw 2>&1 > nwchem-h2-mc-6-31g-${R}.out
  energy_h2_mc_6_31g=`grep "Total MCSCF energy" nwchem-h2-mc-6-31g-${R}.out | tail -n 1 | awk '{ print $5 }'`
  #rm mcscf_dat.*
  cat << EOF > input-h2-mc-sto-3g.nw
echo
start mcscf_dat
geometry units au noautoz
  H 0 0 -$R12
...
task mcscf
EOF
  #$NWCHEM_EXE input-h2-mc-sto-3g.nw 2>&1 > nwchem-h2-mc-sto-3g-${R}.out
  energy_h2_mc_sto_3g=`grep "Total MCSCF energy" nwchem-h2-mc-sto-3g-${R}.out | tail -n 1 | awk '{ print $5 }'`
  #rm hf_dat.*
  cat << EOF > input-h2-hf-6-31g.nw
echo
start hf_dat
geometry units au noautoz
  H 0 0 -$R12
...
task scf energy
EOF
  #$NWCHEM_EXE input-h2-hf-6-31g.nw 2>&1 > nwchem-h2-hf-6-31g-${R}.out
  energy_h2_hf_6_31g=`grep "Total SCF energy" nwchem-h2-hf-6-31g-${R}.out | tail -n 1 | awk '{ print $5 }'`
  #rm hf_dat.*
  cat << EOF > input-h2-hf-sto-3g.nw
echo
start hf_dat
geometry units au noautoz
  H 0 0 -$R12
...
task scf energy
EOF
  #$NWCHEM_EXE input-h2-hf-sto-3g.nw 2>&1 > nwchem-h2-hf-sto-3g-${R}.out
  energy_h2_hf_sto_3g=`grep "Total SCF energy" nwchem-h2-hf-sto-3g-${R}.out | tail -n 1 | awk '{ print $5 }'`
  #rm wfn1_dat.*
  #echo "input-h2-wf-6-31g-${R}.nw nwchem-h2-wf-6-31g-${R}.out ${R}" >> job_list.txt
  echo "input-h2-wf-6-31g-${R}.nw nwchem-h2-wf-6-31g-${R}.out ${R}" >> job_list_h2_6_31g.txt
  cat << EOF > input-h2-wf-6-31g-${R}.nw
echo
start wfn1_dat
geometry units au noautoz
  H 0 0 -$R12
...
task wfn1 energy
EOF
  #echo "input-h2-wf-sto-3g-${R}.nw nwchem-h2-wf-sto-3g-${R}.out ${R}" >> job_list.txt
  echo "input-h2-wf-sto-3g-${R}.nw nwchem-h2-wf-sto-3g-${R}.out ${R}" >> job_list_h2_sto_3g.txt
  cat << EOF > input-h2-wf-sto-3g-${R}.nw
echo
start wfn1_dat
geometry units au noautoz
  H 0 0 -$R12
...
task wfn1 energy
EOF
  echo $R $energy_h2_hf_6_31g  >> results_h2_hf_6_31g.dat
  echo $R $energy_h2_hf_sto_3g >> results_h2_hf_sto_3g.dat
  echo $R $energy_h2_mc_6_31g  >> results_h2_mc_6_31g.dat
  echo $R $energy_h2_mc_sto_3g >> results_h2_mc_sto_3g.dat
  R=`echo "$R + 0.25" | bc -l`
