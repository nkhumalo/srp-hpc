#!/bin/bash
#
# TAU_F90_ABSOLUTE: The absolute path name of the Fortran compiler. This is required
...
if [ ${#USE_BRAIN} -eq 0 ]; then
  ...
  if [ ${#USE_TAU} -eq 0 ]; then
    ...
    if [ ${USE_TAU} == "source" ]; then
      ...
      echo "HVD: call source" > /dev/stderr
      tau_f90.sh -g $@
    elif [ ${USE_TAU} == "compiler" ]; then
      # export F90=/opt/cray/pe/craype/2.5.12/bin/ftn
      echo "HVD: call compiler" > /dev/stderr
      tauf90 -g $@
    else
